...
class Central_ID_Bank(object):
    ...
    def query_user_index(self, user_id):           # user_id字典映射，输入真实id返回index
    ...
    def query_user_id(self, user_index):           # 输入index返回真实id
...
class MetaMarket_DataLoader(object):
    ...
    def __init__(self, task_list, sample_batch_size, task_batch_size=2, shuffle=True, num_workers=8, collate_fn=None,
...
class TaskGenerator(object):
    ...
    def __init__(self, train_data, id_index_bank):
        ...
        if train_data is not None: 
            ...
            self.negatives_train = self._sample_negative(self.ratings)  # 字典，每个user_id的负样本列表
    ...
    def _sample_negative(self, ratings):
        ...
        for userid, group_frame in by_userid_group:
            ...
            neg_itemids = self.item_pool - pos_itemids      # 没有交互过的都是负样本
    ...
    def instance_a_market_train_task(self, index, num_negatives):
        ...
        users, items, ratings = [], [], []
        train_ratings = self.train_ratings  # 交互的表
        for row in train_ratings.itertuples():  # 一个正样本，num_negatives个负样本
            users.append(int(row.userId))
            items.append(int(row.itemId))
            ratings.append(float(row.rating))
            
            cur_negs = self.negatives_train[int(row.userId)]                        # 当前用户的负样本
            cur_negs = random.sample(cur_negs, min(num_negatives, len(cur_negs)) )  # 随机抽四个负样本
            for neg in cur_negs:
                users.append(int(row.userId))
    ...
    def instance_a_market_train_dataloader(self, index, num_negatives, sample_batch_size, shuffle=True, num_workers=8):
    ...
    def load_market_valid_run(self, valid_run_file):  # 把一行item_id分别拆到user_id中,构成<user_id,item_id>pair
        users, items, ratings = [], [], []
    ...
    def instance_a_market_valid_dataloader(self, valid_run_file, sample_batch_size, shuffle=False, num_workers=8):
